...
/*
 * @Retention
 * 		- 어노테이션이 사용되는 시점을 지정한다.
 * 		- 사용자정의 어노테이션은 거의 대부분 프로그램이 실행될 때 해당 어노테이션이 사용되게 된다.
 * 		- 종류
 * 			RetentionPolicy.SOURCE 	- 소스 상태에서 어노테이션이 사용됨
 * 			RetentionPolicy.CLASS 	- 컴파일 시점에 어노테이션이 사용됨
 * 			RetentionPolicy.RUNTIME - 프로그램 실행시점에 어노테이션이 사용됨
 * @Target
 * 		- 어노테이션이 적용되는 곳을 지정한다.
